...
public class YellowMonster extends Enemy {
    ...
    public YellowMonster(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        this.stateTime = stateTime;
        frames = new Array<TextureRegion>();
        frames.add(new TextureRegion(screen.getAtlas().findRegion("yellowmonster"), 0, -2, 22, 20));
        frames.add(new TextureRegion(screen.getAtlas().findRegion("yellowmonster"), 23, -2, 22, 20));
        walkAnimation = new Animation<TextureRegion>(0.2f, frames);
        stateTime = 0;
        setBounds(getX(), getY(), 22 / Maia.PPM, 18 / Maia.PPM);
        setToDestroy = false;
        destroyed = false;
    }
    ...
    public void update(float dt) {
        ...
        if (setToDestroy && !destroyed) {
            ...
            destroyed = true;
            setRegion(new TextureRegion(screen.getAtlas().findRegion("yellowmonster"), 45, -2, 22, 20));
            stateTime = 0;
        } else if (!destroyed) {
    ...
    public void hitOnHead() {
        ...
        Maia.manager.get("audio/sounds/yellowhit.mp3", Sound.class).play();
        setToDestroy = true;
        Hud.addScore(100);
    }
}